...
from ..utils import allowed_file, ALLOWED_PHOTO_EXTENSIONS
...
class ProfileForm(Form):
    ...
    birth_day = DateField(_('Birth Day'), format='%Y-%m-%d')
    ...
    def validate_avatar_file(form, field):
        if field.data and not allowed_file(field.data.filename):
            raise ValidationError("Please upload files with extensions: %s" % "/".join(ALLOWED_PHOTO_EXTENSIONS))
